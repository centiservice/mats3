 ...
 * <li>The {@link JmsMatsJmsSessionHandler_Pooling} must be set up with multiple pools, that is, anything other than
 ...
 * {@link MatsInitiator}, and gets its own Session Pool.</li>
 * <li>When this is finished, it cleans up the Pool that had the sessions for the StageProcessors since these
 ...
 * created to hold their Sessions.</li>
 ...
 * sitting there (at least that's what's supposed to happen!).</li>
 * <li>Finally, the {@link #c_Perform_Futurizer_Request()} method is run, which is identical to the first request. This
 * also sets up the MatsEndpoint, creating a new pool - <i>but the MatsFuturizer it can reuse the MatsInitiator pool,
 * since it sits there available.</i></li>
 * <li>The bug used to be that when it got hold of this Session, it was quite often closed!</li>
 * <li>(Since there is only a quite small chance that the problem happens during each of no-op methods, there are 9 of
 * them between the "a" and "c" method, which seems to be enough compound risk! However, as mentioned, you must "run
 * until fail" to be sure. After is has happened, the closed session situation persist until the "c" method is executed,
 * and thus the test fails.)</li>
 * </ol>
 * <p>
 ...
 * Connection system - which we can se in a log line stating
 ...
 * fails with a "Session is closed" exception when it depools it and tries to use it.
 * </p>
 * <p>
 ...
 * this fast cycling, and probably because the Pooler did something not quite semantically correct. It should clearly
 * not allow the VMTransport to be closed when there is actually is a JMS Connection left! It even specifically
 * <i>closes</i> this JMS Connection as a cleanup procedure, implying that the data structures actually know about the
 * remaining JMS Connection! It might be that in certain conditions the Pooler tried, and managed, to register a new
 * Session on a JMS Connection that was <i>on its way</i> to be closed, or something like this. I have not managed to
 * hunt down the specific ordering of concurrent events that trigger this situation, neither by manual reasoning in the
 * code, nor by making a specific test using only Connections and Session opening and closing using multiple threads.
 * But I now believe that the specific scenario above triggers a race condition present in the Pooler - which then again
 * manages to trigger the ActiveMQ problem - and I have fixed this race condition in the Pooler.
 * </p>
 * <p>
 ...
 * the <code>removeSessionHolderFromPool_And_DitchPoolIfEmpty(SessionHolder)</code> method. Then run this test ("run
 * until fail"). It should fail at the {@link #c_Perform_Futurizer_Request()} method - and if you search the log for
 * "Shutting down VM connectors for broker", you should see that it is this that triggers the problem (you'll also find
 * an exception "TransportDisposedIOException: peer (vm://MatsTestActiveMQ_p87NUR#5) stopped" when the JMS Connection is
