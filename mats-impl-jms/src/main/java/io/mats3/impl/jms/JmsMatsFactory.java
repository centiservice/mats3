...
public class JmsMatsFactory<Z> implements JmsMatsStatics, JmsMatsStartStoppable, MatsFactory {
    ...
    private static final String MATS_IMPLEMENTATION_VERSION = "0.19.14-2023-12-11";
     ...
     * original destination name with "DLQ.". The {@link Function} is invoked with the original JMS Destination name,
     * and should return the DLQ destination name. The method can return a constant to always use the same DLQ: It might
     * be of interest to direct dead letters to the same address that is the default global DLQ for the broker in use;
     * E.g. Apache ActiveMQ Classic has default global DLQ: "ActiveMQ.DLQ", and Apache ActiveMQ Artemis has "DLQ".
     * However, this is very much not recommended - individual dead letter queues should be used for each endpoint.
     *
     ...
     */
    public void setDestinationNameModifierForDlqs(Function<JmsMatsStage<?, ?, ?, ?>, String> destinationNameModifierForDlqs) {
     ...
     */
    void setCurrentMatsFactoryThreadLocal_NestingWithinStageProcessing(MatsTrace<Z> matsTrace, JmsMatsStage<?, ?, ?, Z> currentStage,
            MessageConsumer messageConsumer) {
